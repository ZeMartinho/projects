...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = 0; i < segment.size() - 1; i += 1) {
            Road path = _map.getLabel(segment.get(i), segment.get(i + 1));
            if (i < segment.size() - 2) {
                Road nextRoad = _map.getLabel(
                        segment.get(i + 1), segment.get(i + 2));
                if (path.toString().equals(nextRoad.toString())
                        && path.direction() == nextRoad.direction()) {
                    dist += path.length();
                } else {
                    System.out.printf(seq + ". Take "
                            + path.toString() + " "
                            + path.direction().fullName()
                            + " for %.1f miles.", dist + path.length());
                    System.out.println();
                    dist = 0.0;
                    seq += 1;
                }
                ...
                System.out.printf(seq + ". Take "
                        + path.toString() + " " + path.direction().fullName()
                        + " for %.1f miles to %s.", dist + path.length(),
