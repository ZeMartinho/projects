...
class Rule {
    ...
    private void checkFinishedDependencies() {
        for (int depend :_depends.successors(_vertex)) {
            if (_depends.getLabel(depend).isUnfinished()) {
                error("Error: Did not build dependencies.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        if (_maker.getInitialAge(this.getTarget()) == null) {
            return true;
        }
        for (int depend :_depends.successors(_vertex)) {
            if (_depends.getLabel(depend).outOfDate()) {
                return true;
            } else if (getTime() < _depends.getLabel(depend).getTime()) {
                return true;
            }
        }
        return false;
